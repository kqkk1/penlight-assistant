 ...
 * Ultimate Master Edition v3.3.0 - Perfect Sync & Optimized Build
 */
(() => {
  "use strict";
  ...
  const CONFIG = {
    ...
    PRESETS: {
      "765AS": [
        { n: "765PRO ALLSTARS", c: "#E22B30", p: "R1-5", role: "brand", unit: "", c_v: "", p_v: "" },
        { n: "音無小鳥", c: "#FFF98E", p: "Y3-1", role: "staff", unit: "", c_v: "", p_v: "" }
      ],
      "ミリオン": [
        { n: "765 MILLIONSTARS", c: "#FFC30B", p: "Y1-2", role: "brand", unit: "", c_v: "", p_v: "" },
        { n: "青羽美咲", c: "#008538", p: "G1-2", role: "staff", unit: "", c_v: "", p_v: "" }
      ],
      "学マス": [
        { n: "初星学園", c: "#FF7700", p: "O1-6", role: "brand", unit: "", c_v: "", p_v: "" },
        { n: "根緒亜紗里", c: "#008538", p: "G1-2", role: "staff", unit: "", c_v: "", p_v: "" }
      ],
      "デレマス": [
        { n: "CINDERELLA GIRLS", c: "#2682C6", p: "B2-6", role: "brand", unit: "", c_v: "", p_v: "" },
        { n: "千川ちひろ", c: "#008538", p: "G1-2", role: "staff", unit: "", c_v: "", p_v: "" }
      ],
      "シャニマス": [
        { n: "283プロダクション", c: "#80DEEA", p: "GB6-2", role: "brand", unit: "", c_v: "", p_v: "" },
        { n: "七草はづき", c: "#008538", p: "G1-2", role: "staff", unit: "", c_v: "", p_v: "" }
      ],
      "SideM": [
        { n: "315プロダクション", c: "#00BA74", p: "G3-5", role: "brand", unit: "", c_v: "", p_v: "" },
        { n: "山村賢", c: "#008538", p: "G1-2", role: "staff", unit: "", c_v: "", p_v: "" }
      ],
      "876": [],
      "その他": []
    }
  ...
  class Store {
    ...
    getUnits() {
      return Array.from(new Set((this.cache[this.currentBrand] || []).map(it => it.unit).filter(Boolean)));
    }
  }
  ...
  class DataFetcher {
    ...
    static async fetch(sheetName, force = false) {
      ...
      try {
        ...
        if (CONFIG.PRESETS[sheetName]) {
          CONFIG.PRESETS[sheetName].forEach((item, idx) => {
            if (!seen.has(item.n)) { 
              seen.add(item.n); 
              items.push({ ...item, brand: sheetName, rawIndex: -100 + idx }); 
            }
          });
        }
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          if (sheetName !== "765AS" && name === "天海春香") return [];
          ...
          if (name && p_norm && !seen.has(name)) {
            seen.add(name);
            let role = "idol";
            if (sheetName !== "876" && sheetName !== "その他") {
              if (/ブランド|プロ|プロダクション|学園/i.test(name)) role = "brand";
              else if (/事務員|小鳥|美咲|ちひろ|はづき|山村|亜紗里|担任|講師|社長/i.test(name)) role = "staff";
              else if (/ユニット|色|カラー|代表/i.test(name) || /ユニット|代表/i.test(unit)) role = "unit_color";
            }
  ...
  class UIManager {
    ...
    renderList() {
      ...
      this.els.memberList.innerHTML = list.map(it => {
        ...
        const showUnitTag = it.unit && it.unit !== "ユニット" && it.role === "idol" && !it.n.includes(it.unit);
